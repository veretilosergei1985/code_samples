...
use CardPrinterService\Service\Builder\CardImage\PassengerFirstNameBuilder;
use CardPrinterService\Service\Builder\CardImage\SimpleTextBuilder;
use CardPrinterService\Service\ImageHelper;

class FirstNameNotAnonymousElementPositionBuilder implements ColoredElementPositionBuilderInterface
{
    ...
    public function supports(Template $template, ElementDto $element): bool
    {
        return $element->getType() === SimpleTextBuilder::TYPE
            && $element->getFieldType() === PassengerFirstNameBuilder::FIELD_TYPE
    ...
    {
        $sizes = $this->imageHelper->getTextSize($element, $passengerDto->firstName);
        $points[] = [
            'x' => $element->getX(),
            'y' => $element->getY(),
            'w' => $sizes['width'],
